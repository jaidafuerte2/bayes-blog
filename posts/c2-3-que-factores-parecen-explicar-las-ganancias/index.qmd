---
title: "¿Qué factores parecen explicar las ganancias?"

description: "Un modelo bayesiano con Superstore permite explorar cómo descuento, ventas, cantidad y categoría se asocian con la rentabilidad."
...
* `Discount`: porcentaje de descuento aplicado al pedido.
* `Sales`: valor monetario de las ventas.
* `Quantity`: cantidad de productos incluidos.
* `Category`: categoría del pedido.
...
coeficientes <- readRDS("../../R/col2-3/coeficientes.rds")
intervalos <- readRDS("../../R/col2-3/intervalos.rds")
draws <- readRDS("../../R/col2-3/draws.rds")
predicciones <- readRDS("../../R/col2-3/predicciones.rds")
```
...
> Manteniendo constantes `Sales`, `Discount` y `Category`, una unidad adicional de `Quantity` está asociada con aproximadamente **$3.23 menos de Profit por pedido**.
...
* `Office Supplies`: aproximadamente **+$50.25** frente a Furniture.
* `Technology`: aproximadamente **+$41.39** frente a Furniture.
...
* `Sales = $54.50`, la mediana observada;
* `Quantity = 3`, la mediana observada;
